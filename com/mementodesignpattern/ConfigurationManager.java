package com.mementodesignpattern;
...
public class ConfigurationManager {
    ...
    public void saveState(ApplicationConfiguration appConfig) {
        ...
        System.out.println(history.size() == 1 ? "[+] Default State: " + configurationMemento
    ...
    public void undo(ApplicationConfiguration appConfig) {
        if (history.size() > 1) {
            ...
            appConfig.restore(mementoToBeRestored); // restores the application configuration to the previous saved
                                                    // state
            System.out.println("[+] Undo performed. History size: " + history.size());
            System.out.println(history.size() == 1 ? "[+] Default State: " + mementoToBeRestored
                    : "[+] Current State: " + mementoToBeRestored);
        } else {
